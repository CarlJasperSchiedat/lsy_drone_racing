import numpy as np
from casadi import MX, Function, DM, vertcat, cos, sin, exp, inf, sumsqr, nlpsol, dot, norm_2, fmax
...
def apply_segment_tendency(N_list, shorten=[0, 2], lengthen=[1, 3], max_shift=0.1):
    ...
    for i in shorten:
        deltas[i] -= np.random.uniform(0.02, max_shift)
    for i in lengthen:
        deltas[i] += np.random.uniform(0.02, max_shift)
...
def optimize_waypoint_positions(gates, gates_quat, N_list, obstacles, v_start, v_end, dt=1/50):
    ...
    for seg_idx, N_seg in enumerate(N_list):
        ...
        for j in range(N_seg):
            ...
            # --------------------------------Cost------------------------------------
            cost += sumsqr(ui) # Control effort minimization
            cost += 1e-0 * sumsqr(xi_next[:3] - xi[:3]) # Position change minimization
            if obstacles: # Obstacle penalty
        ...
        # Zwischen-Gates: Position fixieren, Velocity frei
...
def optimize_full_trajectory(gates, gates_quat, obstacles, v_start, v_end, t_min, t_max, step=1, dt=1/50):
    ...
    for N in range(N_min, N_max + 1, step_it):
        ...
        for _ in range(5):
            ...
            N_list = apply_segment_tendency(N_list, shorten=[0,2], lengthen=[1,3], max_shift=0.2)
            # print(N_list)
            try:
                pos, cost = optimize_waypoint_positions(gates, gates_quat, N_list, obstacles, v_start, v_end, dt=dt)
                if cost < best_cost:
                    best_cost = cost
                    best_X_opt = pos
                    best_N = N_list
